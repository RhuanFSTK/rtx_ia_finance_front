document.addEventListener("DOMContentLoaded", () => {
  ...
  // Função para mostrar e esconder spinner e cards
  ...
  // Função para mostrar resultados com alerta estilizado
  function mostrarResultado(container, tipo, mensagem) {
    container.classList.remove("alert-success", "alert-danger", "alert-warning", "alert-info", "alert-light");
    container.classList.add("alert", `alert-${tipo}`, "fade", "show");
    container.innerHTML = mensagem;
    container.classList.remove("d-none");
  }

  // Função para enviar arquivos à API
  ...
  async function tirarFoto() {
    try {
      ...
      // Espera 1s para câmera estabilizar
  ...
  // Envia texto da descrição para a API
    ...
    try {
      ...
      if (res.ok) {
        mostrarResultado(
          resultadoTexto,
          "success",
          `<strong>Classificação:</strong> ${data.classificacao} <br /> <strong>Valor:</strong> R$ ${parseFloat(data.valor).toFixed(2)}`
        );
      } else {
        mostrarResultado(
          resultadoTexto,
    ...
    try {
      ...
      if (res.ok) {
        if (data.gastos.length === 0) {
          mostrarResultado(resultadoConsulta, "warning", "Nenhum gasto encontrado no período.");
        } else {
          let tabelaHtml = `
          <div class="alert alert-success">Total gasto: R$ ${data.total.toFixed(2)}</div>
        ...
        mostrarResultado(resultadoConsulta, "danger", `Erro: ${data.detail}`);
      }
    } catch (err) {
      mostrarResultado(resultadoConsulta, "danger", `Erro ao consultar: ${err.message}`);
    }
  });
  ...
  capturaFotoBtn.addEventListener("click", () => {
    tirarFoto();
  });
  ...
  arquivoInput.addEventListener("change", async () => {
    ...
    if (file.type.startsWith("audio/")) {
      ...
      mostrarResultado(resultadoAudioImagem, "warning", "Tipo de arquivo não suportado.");
      resultadoAudioImagem.classList.remove("d-none");
      waveformContainer.classList.add("collapse");
      waveformContainer.classList.remove("show");
    }
