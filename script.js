    ...
    try {
      ...
      if (res.ok) {
        mostrarResultado(
          resultadoTexto,
          "success",
          `<strong>Classificação:</strong> ${data.classificacao} <br /> <strong>Valor:</strong> R$ ${parseFloat(
            data.valor
          ).toFixed(2)}`
        );
      } else {
        mostrarResultado(resultadoTexto, "danger", `<strong>Erro:</strong> ${data.detail}`);
      }
    } catch (err) {
      hideLoading();
      mostrarResultado(resultadoTexto, "danger", `<strong>Erro:</strong> ${err.message}`);
    }
  });
